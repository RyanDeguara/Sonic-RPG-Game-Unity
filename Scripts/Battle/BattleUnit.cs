...
public class BattleUnit : MonoBehaviour
{
    ...
    public Hedgehog Hedgehog { get; set; }
    ...
    public void Setup(Hedgehog hedgehog)
    {
        Hedgehog = hedgehog;
        if (isPlayerUnit)
        {
            image.sprite = Hedgehog.Base.BackSprite;
        }
        else
        {
    ...
    public void PlayAttackAnimation()
    {
        ...
        if (isPlayerUnit)
        {
            sequence.Append(image.transform.DOLocalMoveY(originalPos.y + 40f, 0.25f));
        }
        else
        {
